...
def redirectConvSecounce(listOfConv,memorize,chat_property,fullConersition):
    convItem = None
    response  = []
    for itemOfCon in listOfConv:
        if itemOfCon["intent"] == "any":
            for item in fullConersition:
                if item['intent']== itemOfCon["brackIntent"]:
                    try:
